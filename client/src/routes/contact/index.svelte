<script>
	...
	onMount(async () => {
		const url = 'http://localhost:1337/api/contact?populate=*';
		contactContent = await getPayload(url);
		loading = false;
	});
</script>
			...
			<div class="bg-gray-50 py-16 px-4 sm:px-6 lg:col-span-2 lg:px-8 lg:py-24 xl:pr-12">
				<div class="max-w-lg mx-auto">
					...
					<p class="mt-3 text-lg leading-6 text-gray-500">
						{contactContent.attributes.Content}
					</p>
					<dl class="mt-8 text-base text-gray-500">
						<div>
							<dt class="sr-only">Regional Location</dt>
							<dd>
								<p>{contactContent.attributes.Location}</p>
							</dd>
						</div>
						<div class="mt-3">
							...
							<dd class="flex">
								...
								<span class="ml-3"> {contactContent.attributes.contactEmail} </span>
							</dd>
						</div>
					</dl>
				</div>
